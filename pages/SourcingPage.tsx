        ...
        <div className={`space-y-6 transition-all duration-500 ${loading ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
            ...
            <div className="lg:col-span-5 !bg-white dark:!bg-slate-800 rounded-[32px] p-8 shadow-xl border border-slate-100 dark:border-slate-700 relative overflow-hidden flex flex-col justify-between min-h-[320px]">
                ...
                {imagePreview && hasData && (
                    <div className="absolute top-6 right-6 w-20 h-20 rounded-xl border-2 border-white dark:border-slate-600 shadow-md overflow-hidden">
                ...
                <div className="mt-12">
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">AI Market Verdict</div>
                    <div className={`text-5xl sm:text-6xl font-black tracking-tight leading-none mb-3 transition-all ${verdictStyle.color}`}>
                    {verdictStyle.label}
                    </div>
                ...
                <div className="mt-8 flex flex-col gap-3">
                   {hasData ? (
                      <>
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Saturation</span>
                           <span className="text-base font-black text-slate-800 dark:text-slate-200">{scoutResult.vitals?.saturation || '-'}</span>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Liquidity</span>
                           <span className="text-base font-black text-slate-800 dark:text-slate-200">{scoutResult.vitals?.liquidity || '-'}</span>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Seasonality</span>
                           <span className="text-base font-black text-slate-800 dark:text-slate-200">{scoutResult.vitals?.trend || '-'}</span>
                        </div>
                      </>
                   ) : (
                      <div className="text-center text-xs text-slate-300 py-4 font-medium uppercase tracking-wider">Awaiting Market Data...</div>
                   )}
                </div>

                <div className={`mt-6 p-4 rounded-2xl border transition-all ${hasData ? 'bg-slate-50 dark:bg-slate-900/50 border-slate-100 dark:border-slate-700' : 'bg-slate-50/50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800'}`}>
