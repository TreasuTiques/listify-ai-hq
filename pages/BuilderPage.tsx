...
import { useNavigate } from 'react-router-dom'; // Assuming you use router, or we use props

// If you are not using React Router's hook, we can remove useNavigate
// But typically for redirects we need it. For now, we'll just use window.location if needed.
...
const BuilderPage: React.FC = () => {
  ...
  // 2. THE SAVE FUNCTION (The New Engine)
  const handleGenerateAndSave = async () => {
    ...
    if (!title) {
      alert("Please enter a title first (or wait for AI to generate one).");
      return;
    }
    ...
    try {
      // A. Insert into Supabase "listings" table
        ...
        .insert([
          {
            user_id: user.id,
            title: title,
            brand: brand, // We need to add this column to DB later if we want it separate, or put in description
            condition: condition,
            platform: activePlatform,
            status: 'draft',
            description: description || 'Generated by Listify AI', // Placeholder for now
          }
        ])
        .select();
      ...
      // B. Success!
      alert("Listing Saved to Dashboard! 🚀");
