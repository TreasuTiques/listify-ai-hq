...
import { generateListingFromImages } from '../services/ai'; // ✅ Correct Plural Import
...
const BuilderPage: React.FC = () => {
  ...
  const runAIAnalysis = async (files: File[], platform: string, proMode: boolean, cond: string) => {
    ...
    try {
      ...
      // Keep user selected condition, don't overwrite with AI guess
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent) => {
    ...
    // If condition is ALREADY selected (e.g. adding more photos later), we update.
    if (condition) {
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-8">
            ...
            <div className="mb-6 relative">
              ...
              <div className="relative">
                {/* ✅ FIX: Increased right padding to pr-16 so button doesn't block text */}
                <input 
                  type="text" 
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 pr-16 font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all placeholder:text-slate-400"
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                 ...
                 {activePlatform === 'ebay' ? (
                    ...
                    // ✅ FIX: Added Copy Button for NON-eBay platforms too
                    <button onClick={() => copyToClipboard(description, 'desc')} className="bg-blue-50 hover:bg-blue-100 text-blue-600 px-3 py-1 rounded-lg text-[10px] font-bold uppercase flex items-center gap-1 transition-all border border-blue-100">
              ...
              {activePlatform === 'ebay' ? (
                ...
                <div className="relative">
                   {editorTab === 'visual' ? (
                      // ✅ FIX: Height increased to h-[600px]
                      <div 
                         className="w-full bg-white border border-slate-200 rounded-xl px-4 py-4 h-[600px] overflow-y-auto prose prose-sm max-w-none"
                ...
                <textarea 
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-slate-900 focus:outline-none focus:border-blue-500 transition-all placeholder:text-slate-400 h-[600px] resize-none"
