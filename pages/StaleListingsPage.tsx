...
const StaleListingsPage: React.FC<StaleListingsProps> = ({ isGuest = false, onNavigate }) => {
   ...
   // 🛡️ SECURITY & USAGE STATE
   const [scanCount, setScanCount] = useState(0);
   const SESSION_LIMIT = 50; // Safety cap for QA session
   ...
   const checkUsageLimit = () => {
      if (scanCount >= SESSION_LIMIT) {
         alert("⚠️ QA Safety Limit Reached. Please refresh to reset or contact Admin.");
         return false;
      }
      return true;
   };
   ...
   const runDiagnosis = async () => {
      if (!checkUsageLimit()) return; // 🛑 STOP IF OVER LIMIT
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden relative mb-12">
               ...
               <div className="p-8">
                  ...
                  <button
                     ...
                     disabled={isLoading || (inputMode === 'xray' && selectedFiles.length === 0) || (inputMode === 'text' && !manualTitle)}
